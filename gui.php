...
*/
	...
	function writeJSboard()
	{
		...
		for ($i = 0; $i < 8; $i++)
		{
			...
			for ($j = 0; $j < 8; $j++)
			{
				echo (isset($board[$i][$j]) ? $board[$i][$j] : 0);
				if($j < 7)
					echo (', ');
			}
			echo ("]");
	...
	function writeVerbousHistory()
	{
		...
		for ($i = 0; $i <= $numMoves; $i++)
		{
			...
			$moves[floor($i/2)][$i & 1] = moveToVerbousString($history[$i]['curColor'], $history[$i]['curPiece'], $history[$i]['fromRow'], $history[$i]['fromCol'], $history[$i]['toRow'], $history[$i]['toCol'], $tmpReplaced, $tmpPromotedTo, $tmpCheck);
		}
	...
	function writeHistoryPGN()
	{
		...
		for ($i = 0; $i <= $numMoves; $i++)
		{
			...
			$moves[floor($i/2)][$i & 1] = moveToPGNString($history[$i]['curColor'], $history[$i]['curPiece'], $history[$i]['fromRow'], $history[$i]['fromCol'], $history[$i]['toRow'], $history[$i]['toCol'], $tmpReplaced, $tmpPromotedTo, $tmpCheck);
		}
	...
	function writeHistory()
	{
		...
		for ($i = 0; $i < count($moves); $i++)
		{
			...
			if($i % 4 == 0) // Four moves on each line
				echo("\n");
			echo ("['" . (isset($moves[$i][0]) ? $moves[$i][0] : "") . "', '" . (isset($moves[$i][1]) ? $moves[$i][1] : "") . "']");
			$comma = ', ';
		}
		echo("];\n");
	}
	...
	function writePGN()
	{
		...
		global $simplemove,$isDraw,$move_turn,$move_PlyNumber, $CFG_TABLE, $dbh;
